...
/** Dijkstra's Shortest Path Algorithm used to sum multiple missing sensors.
...
* Sensor 1 is S10, 2 is S04, 3 is S013, 4 is S015, 7 is S025, 19 is S037, 8 is S050, ... 18 is S021
* Parameters:
...
*/
//helper min function 
int minDistance2(double dist[], bool sptSet[])
{
...
void printSolution2(double dist[], int n) {
...
double dijkstras2(double graph[V][V], int src, int tosensor) {
	...
	if (src == 201) { //201 denotes sensor 37
		src = 19;
	}
	if (tosensor == 201) {
		tosensor = 19;
	...
	dist[src] = 0;
	for (int k = 0; k < V - 1; k++) {
		int u = minDistance2(dist, sptSet);
		sptSet[u] = true;
		for (int v = 0; v < V; v++) {
