...
public:
   ...
   static std::string& ConvertToString(const tstring& strIn, std::string& strOut)
   {
      ...
      else
      {
         ...
         //no longer get error/warning here
         wcstombs_s(NULL, &*strOut.begin(), strOut.size(), strIn.c_str(), strOut.size());
      }
   #else
      strOut = strIn;
   #endif
   ...
   static tstring& ConvertToTString(const char* szIn, tstring& strOut)
   {
      ...
      else
      {
         ...
         //length is probably different from what sizeInWords parameter is supposed to be
         mbstowcs_s(NULL, &*strOut.begin(), len, szIn, len);
      }
   #else
      strOut = szIn;
   #endif
